...
namespace AES_Encryption
{
    ...
    {
        const int Nb = 4;
        const int AES_BLOCKLEN = 16; //Block length in bytes AES is 128b block only
...
const int Nr = 14;
...
#endif
#if AES128
    const int Nk = 4;        // The number of 32 bit words in a key.
    const int Nr = 10;      // The number of rounds in AES Cipher.
#endif

        public State_t state_t = new State_t();
        AES_ctx aes_ctx_roundKey = new AES_ctx();
  ...
  0x17, 0x2b, 0x04, 0x7e, 0xba, 0x77, 0xd6, 0x26, 0xe1, 0x69, 0x14, 0x63, 0x55, 0x21, 0x0c, 0x7d };
        ...
        uint[] Rcon = new uint[11] { 0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };
        /*
        ...
        private uint getSBoxValue(uint num)
        {
            return (sbox[(num)]);
        }

        private uint getSBoxInvert(uint num)
        {
            return rsbox[num];
        }
    ...
    if (i % Nk == 4)
    {
      // Function Subword()
      {
        ...
        private void ShiftRows(ref State_t state)
        {
            uint temp;

            // Rotate first row 1 columns to left  
            temp = state.state_t[0, 1];
            state.state_t[0, 1] = state.state_t[1, 1];
            state.state_t[1, 1] = state.state_t[2, 1];
            state.state_t[2, 1] = state.state_t[3, 1];
            state.state_t[3, 1] = temp;
            ...
            state.state_t[0, 3] = state.state_t[3, 3];
            state.state_t[3, 3] = state.state_t[2, 3];
            state.state_t[2, 3] = state.state_t[1, 3];
            state.state_t[1, 3] = temp;
        }

        private uint xtime(uint x)
        {
        ...
        private uint Multiply(uint x, uint y)
        {
            return (((y & 1) * x) ^
                 ((y >> 1 & 1) * xtime(x)) ^
                 ((y >> 2 & 1) * xtime(xtime(x))) ^
        ...
        public void AES_ECB_encrypt(ref AES_ctx ctx, ref uint[] buf)
        {
            ...
            Cipher(ref state_t, ref ctx.RoundKey);
        }
        ...
        public void Phex(ref uint[] str, uint pointer)
        {
            uint len = 16;
            uint i;
            for (i = 0; i < len; ++i)
            {
                Console.Write((str[i + pointer]).ToString("X2"));
                Console.Write(" ");
            }
            Console.WriteLine("\n");
        }
    ...
    class AES_ctx
    {
        const int AES_keyExpSize = 176;
        public uint[] RoundKey = new uint[AES_keyExpSize];
#if CBC || CTR
        const int AES_BLOCKLEN = 16;
        public uint[] Iv = new uint[AES_BLOCKLEN];
#endif
    };

    class State_t
    {
        public uint[,] state_t = new uint[4, 4];
    }
}
